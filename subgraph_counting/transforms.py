...
class ToTCONV(BaseTransform):
    ...
    def __call__(self, data: HeteroData):
        ...
        for edge_type in edge_types:
            ...
            A = torch.sparse_coo_tensor(edge_tensor, torch.ones([edge_tensor.shape[-1]], device= edge_tensor.device), (adj_size, adj_size), device= edge_tensor.device, requires_grad= False)
...
class ToTconvHetero(BaseTransform):
    ...
    def __call__(self, data: HeteroData):
        ...
        edge_tensor, node_slices, edge_slices = to_homogeneous_edge_index(data) # ALERT: wil affect the original data

        # sort edge_tensor and get the indices
        sorted_edge_tensor, edge_tensor_sort_indices = pyg_utils.sort_edge_index(edge_tensor, edge_attr=torch.tensor([i for i in range(edge_tensor.shape[-1])], device=edge_tensor.device))


        adj_size = max(node_slices.values(), key= lambda x: x[1])[1] # maximum node indicies

        A = torch.sparse_coo_tensor(edge_tensor, torch.ones([edge_tensor.shape[-1]], device= edge_tensor.device), (adj_size, adj_size), device= edge_tensor.device, requires_grad= False)
        A2 = torch.sparse.mm(A, A)
        Tri_And_Edge = (A*A2 + A).coalesce() # Triangle = A*A2, Tri_And_Edge is sorted for sure
        # edge_weight = Triangle[edge_index] # can't do it :(
        ...
        tri_edge_index = tri_edge_index.gather(dim=-1, index=edge_tensor_sort_indices.argsort())
        ...
        for edge_type, slices in edge_slices.items():
            ...
            edge_tensor_this_type[0,:] = edge_tensor_this_type[0,:] - node_slices[src_type][0] # map to node indicies of the corresponding type
            edge_tensor_this_type[1,:] = edge_tensor_this_type[1,:] - node_slices[dst_type][0]

            data[src_type, rel_type+'_triangle', dst_type].edge_index = edge_tensor_this_type[:,tri_edge_index_this_type] # size = 2*#triangle_edge
            data[src_type, rel_type+'_tride', dst_type].edge_index = edge_tensor_this_type[:,~tri_edge_index_this_type] # size = 2*#tride_edge
...
def NetworkxToHetero(nx_graph: Union[nx.Graph, nx.DiGraph], type_key: str = 'type', feat_key: str = 'feat') -> pyg.data.HeteroData:
    ...
    hetero_edge_dict = defaultdict(list) # use (from_node_type, edge_type, to_node_type) to add edge_index
    hetero_node_dict = defaultdict(dict) # use node_type to classify nodes, [node]: new_id
    ...
    for edge in nx_graph.edges:
        ...
        try: 
            ...
            edge_type = 'union'
        hetero_edge_type = (nx_graph.nodes[n0][type_key], edge_type, nx_graph.nodes[n1][type_key])
        hetero_edge_dict[hetero_edge_type].append((hetero_node_dict[nx_graph.nodes[n0][type_key]][n0], hetero_node_dict[nx_graph.nodes[n1][type_key]][n1]))
    ...
    for node_type, node_dict in hetero_node_dict.items():
        ...
        if feat_key not in node_attrs: # add feat Tensor([0]) if no feat exist for nodes
            for node in nx_graph.nodes:
