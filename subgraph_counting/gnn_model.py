...
class BaseGNNCore(nn.Module):
    ...
    def forward(self, x, edge_index, query_emb= None):
        ...
        # TConv Preprocess
        # TConv is the triangle-tride version of SHMP
        if self.conv_type == "TCONV":
            edge_index, _ = pyg_utils.remove_self_loops(edge_index)
            edge_index = pyg_utils.to_undirected(edge_index)
            adj_size = x.shape[0]
            A = torch.sparse_coo_tensor(edge_index, torch.ones([edge_index.shape[-1]], device= edge_index.device), (adj_size, adj_size), device= edge_index.device, requires_grad= False)
            ...
            assert (edge_index == torch.stack(edge_list, dim=0).T).all().item()
            edge_weight = torch.stack(edge_weight,dim=0).unsqueeze(1) # (#E,1)
            edge_weight = edge_weight>1
        # GossipConv Preprocess
        elif self.conv_type == "GOSSIP":
            edge_index, _ = pyg_utils.remove_self_loops(edge_index)
            edge_index = pyg_utils.to_undirected(edge_index)
...
class TConv(pyg_nn.MessagePassing):
    '''
    triangle message passing network
    '''
    def __init__(self, in_channels, out_channels, aggr="add"):
        super(TConv, self).__init__(aggr=aggr)

        self.in_channels = in_channels
        self.out_channels = out_channels

        # self.lin_com = nn.Sequential(nn.Linear(in_channels, out_channels),nn.LeakyReLU(0.1)) # common lin for both inputs
        self.lin_sps = nn.Linear(in_channels, out_channels) # sparse connect nodes (star)
        self.lin_dns = nn.Linear(in_channels, out_channels) # dense connect nodes (triangle)
    ...
    def forward(self, x, edge_index, edge_weight=None, size=None,
        ...
        # find triangles if not found earlier
        if edge_weight == None:
            print("warning: please use TCONV data transform to speedup the computation")
            edge_index, _ = pyg_utils.remove_self_loops(edge_index)
            edge_index = pyg_utils.to_undirected(edge_index)
            adj_size = x.shape[0]
            A = torch.sparse_coo_tensor(edge_index, torch.ones([edge_index.shape[-1]], device= edge_index.device), (adj_size, adj_size), device= edge_index.device, requires_grad= False)
    ...
    def message(self, x_i, x_j, edge_weight):
        return  self.lin_dns(x_j)*edge_weight + self.lin_sps(x_j)*(~edge_weight)
