...
class ERGenerator(dataset.Generator):
    ...
    def generate(self, size=None):
        ...
        p = np.random.beta(
            alpha, beta
        )  # p follows beta distribution with mean = log2(num_nodes) / num_nodes
        ...
        while not nx.is_connected(graph):
            ...
            graph = nx.gnp_random_graph(num_nodes, p)
        logging.debug(
            "Generated {}-node E-R graphs with average p: {}".format(num_nodes, mean)
        )
        return graph
    ...
    def generate(self, size=None):
        ...
        while curr_num_graphs < 1:
            ...
            try:
                ...
                pass
        logging.debug(
            "Generated {}-node W-S graph with average density: {}".format(
...
class PowerLawClusterGenerator(dataset.Generator):
    ...
    def generate(self, size=None):
        ...
        graph = graph.subgraph(c).copy()
        logging.debug(
            "Generated {}-node powerlaw cluster graph with max m: {}".format(
                num_nodes, max_m
            )
        )
        return graph
...
def get_dataset(task, dataset_len, sizes, size_prob=None, **kwargs):
    generator = get_generator(sizes, size_prob=size_prob, dataset_len=dataset_len)
...
def main():
    ...
    print(
        "Example graph: nodes {}; edges {}".format(
            example_graph.G.nodes, example_graph.G.edges
        )
    )

    print(
        "Even the same index causes a new graph to be generated: edges {}".format(
            dataset[0].G.edges
        )
    )

    print(
        "This generator has no label: {}, "
