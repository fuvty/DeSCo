...
def compute_groundtruth(
    train_dataset_name, output_path, force_nx_queries: List[nx.Graph] = None
):
    """
    ...
    if workload.exist_groundtruth(query_ids=query_ids):
        ...
        workload.canonical_count_truth = workload.compute_groundtruth(
            query_ids=query_ids, queries=force_nx_queries, save_to_file=True
        )
    workload.generate_pipeline_datasets(
        depth_neigh=1
    )  # generate pipeline dataset, including neighborhood dataset and gossip dataset

    ground_truth_canonical = workload.canonical_count_truth[
        workload.neighborhood_dataset.nx_neighs_indicator, :
    ]
    ground_truth_graphlet = workload.neighborhood_dataset.aggregate_neighborhood_count(
        ground_truth_canonical
    )

    truth_sum = (
        torch.sum(ground_truth_graphlet, dim=0, dtype=int)
        .detach()
        .cpu()
        .numpy()
        .reshape(-1, 1)
